...
class GroundBox3dCoderTorch(GroundBox3dCoder):
    def encode_paddle(self, boxes, anchors):
        return box_paddle_ops.second_box_encode(boxes, anchors, self.vec_encode,
                                               self.linear_dim)

    def decode_paddle(self, boxes, anchors):
        return box_paddle_ops.second_box_decode(boxes, anchors, self.vec_encode,
                                               self.linear_dim)
...
class BevBoxCoderTorch(BevBoxCoder):
    def encode_paddle(self, boxes, anchors):
        ...
        return box_paddle_ops.bev_box_encode(boxes, anchors, self.vec_encode,
                                            self.linear_dim)

    def decode_paddle(self, encodings, anchors):
        anchors = anchors[..., [0, 1, 3, 4, 6]]
        ret = box_paddle_ops.bev_box_decode(encodings, anchors, self.vec_encode,
                                           self.linear_dim)
        z_fixed = paddle.full([*ret.shape[:-1], 1],
                             self.z_fixed,
                             dtype=ret.dtype)
        h_fixed = paddle.full([*ret.shape[:-1], 1],
                             self.h_fixed,
                             dtype=ret.dtype)
