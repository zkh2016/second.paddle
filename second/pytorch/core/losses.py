...
class WeightedL2LocalizationLoss(Loss):
  ...
  def _compute_loss(self, prediction_tensor, target_tensor, weights):
    ...
    if self._code_weights is not None:
      self._code_weights = paddle.cast(self._code_weights, prediction_tensor.dtype, place=prediction_tensor.place)
...
class WeightedSmoothL1LocalizationLoss(Loss):
  ...
  def _compute_loss(self, prediction_tensor, target_tensor, weights=None):
    ...
    diff = prediction_tensor - target_tensor
    if self._code_weights is not None:
      code_weights = paddle.cast(self._code_weights, prediction_tensor.dtype)
      diff = code_weights.reshape((1, 1, -1)) * diff
    abs_diff = paddle.abs(diff)
    tmp = paddle.full(abs_diff.shape, 1/(self._sigma**2))
    #abs_diff_lt_1 = paddle.cast(paddle.less_equal(abs_diff, 1 / (self._sigma**2)), abs_diff.dtype)
    abs_diff_lt_1 = paddle.cast(paddle.less_equal(abs_diff, tmp), abs_diff.dtype)
...
def _sigmoid_cross_entropy_with_logits(logits, labels):
  ...
  loss = paddle.clip(logits, min=0) - logits * paddle.cast(labels, logits.dtype)
...
class WeightedSigmoidClassificationLoss(Loss):
  ...
  def _compute_loss(self,
    ...
    if class_indices is not None:
      weights *= paddle.cast(indices_to_dense_vector(class_indices,
            prediction_tensor.shape[2]).reshape((1, 1, -1)), prediction_tenso.dtype)
...
class SigmoidFocalClassificationLoss(Loss):
  ...
  def _compute_loss(self,
    ...
    if class_indices is not None:
      weights *= paddle.cast(indices_to_dense_vector(class_indices,
            prediction_tensor.shape[2]).reshape((1, 1, -1)), prediction_tensor.dtype)
    per_entry_cross_ent = (_sigmoid_cross_entropy_with_logits(
        labels=target_tensor, logits=prediction_tensor))
...
class SoftmaxFocalClassificationLoss(Loss):
  ...
  def _compute_loss(self,
    ...
    if self._alpha is not None:
      ...
      paddle.to_tensor(1 - self._alpha, dtype=per_entry_cross_ent.dtype), 
      paddle.to_tensor(self._alpha, per_entry_cross_ent.dtype))
    focal_cross_entropy_loss = (modulating_factor * alpha_weight_factor *
                                per_entry_cross_ent)
    return focal_cross_entropy_loss * weights
...
class WeightedSoftmaxClassificationLoss(Loss):
  ...
  def _compute_loss(self, prediction_tensor, target_tensor, weights):
    ...
    tmp = paddle.full(prediction_tensor.shape, self._logit_scale)
    prediction_tensor = paddle.divide(
        prediction_tensor, tmp)
  ...
  def _compute_loss(self, prediction_tensor, target_tensor, weights):
    ...
    if self._bootstrap_type == 'soft':
      bootstrap_target_tensor = self._alpha * target_tensor + (
          1.0 - self._alpha) * paddle.sigmoid(prediction_tensor)
    else:
      bootstrap_target_tensor = self._alpha * target_tensor + (
          1.0 - self._alpha) * (paddle.sigmoid(prediction_tensor) > 0.5).float()
    per_entry_cross_ent = (_sigmoid_cross_entropy_with_logits(
